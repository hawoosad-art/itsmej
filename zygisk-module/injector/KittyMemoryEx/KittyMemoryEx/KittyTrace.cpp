...
kitty_rp_call_t KittyTraceMgr::_callSyscall(long sysnr, int nargs, ...)
{
    ...
    // vDSO redirect: if the current PC is inside [vdso], PTRACE_POKEDATA silently fails
    // because the vDSO is backed by a special non-writable kernel page.
    // Leaving the original bytes intact means single-step hits whatever the vDSO has there
    // (often a BKPT/svc stub), causing SIGTRAP/CODE=2 which kills the process when delivered.
    // Fix: find a file-backed executable region (e.g. libc.so text) and redirect the
    // syscall injection there.  The syscall arguments/number are in registers so the
    // injection site address does not matter.  Original registers are restored afterwards
    // by _autoRestoreRegs so the process resumes from its real PC.
    // Affects ARM32 (Android 9/10, API 28-29) and AArch64 (Android 13, API 33).
#if defined(__arm__) || defined(__aarch64__)
    {
        auto pcMap = KittyMemoryEx::getAddressMap(_pid, target_pc_mem);
        if (pcMap.isValid() && pcMap.pathname == "[vdso]")
        {
            auto allMaps = KittyMemoryEx::getMaps(_pid);
            for (const auto &m : allMaps)
            {
                // Need a file-backed executable segment with enough room for 4 bytes.
                ...
                thumb = false;
#endif
                syscall_code.assign(std::begin(KittyTraceInsns::SYSCALL),
                                    std::end(KittyTraceInsns::SYSCALL));
                break;
            }
        }
    }
...
bool KittyTraceMgr::setHwBreakpoint(uintptr_t address, KT_HW_BP_TYPE type, KT_HW_BP_SIZE size, int slot)
{
    ...
    int l_bit = (slot * 2);
    int rw_bit = 16 + (slot * 4);
    int len_bit = 18 + (slot * 4);

    dr7 &= ~((3UL << rw_bit) | (3UL << len_bit) | (1UL << l_bit));
    dr7 |= (type_bits << rw_bit) | (len_bits << len_bit) | (1UL << l_bit);
