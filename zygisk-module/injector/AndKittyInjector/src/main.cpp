...
int main(int argc, char *args[])
{
    ...
    program.add_argument("--package")
        .help("Target package name to inject into. (mutually exclusive with --pid)")
    ...
    program.add_argument("--pid")
        .help("Target raw PID to inject into. Bypasses package name resolution. (mutually exclusive with --package)")
        .default_value(0)
        .scan<'i', int>()
    ...
    bool has_package = !inj_cfg.package.empty();
    bool has_pid = target_pid > 0;
    if (has_package == has_pid)
    {
        KITTY_LOGE("Specify exactly one of --package <name> or --pid <pid>.");
        std::cerr << program;
        return 1;
    }
    if (has_pid && (inj_cfg.launch || inj_cfg.watch))
    {
        KITTY_LOGE("--pid cannot be combined with --launch/--watch (no package to (re)launch).");
        return 1;
    }

    KITTY_LOGI("======== INJECTION ARGS ========");
    if (has_pid)
        KITTY_LOGI("pid: %d", target_pid);
    else
    ...
    if (has_pid)
    {
        ...
        injection_ok = inject(target_pid, libs, inj_cfg, &injected_libs_info);
    }
