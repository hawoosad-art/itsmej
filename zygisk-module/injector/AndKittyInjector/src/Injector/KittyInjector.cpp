                ...
                if (!error_str.empty())
                {
                             ...
                             KittyUtils::String::contains(error_str, "Permission denied", false))
                    {
                        KITTY_LOGE("Injector: Maybe use memfd or disable SELinux.");
                             ...
                             KittyUtils::String::contains(error_str, "Permission denied", false))
                    {
                        KITTY_LOGE("Injector: memfd mmap(PROT_EXEC) denied. Dual root cause:");
                        KITTY_LOGE("  1. SELinux: domain lacks execfd. Inject inside setenforce-0 window.");
                        KITTY_LOGE("  2. vm.memfd_noexec>=1 (kernel>=6.3): MFD_EXEC now passed w/ EINVAL fallback.");
                    }
                    ...
                    {
                        KITTY_LOGE("Injector: memfd mmap(PROT_EXEC) denied. Dual root cause:");
                        KITTY_LOGE("  1. SELinux: domain lacks execfd — must inject inside setenforce-0 window.");
                        KITTY_LOGE("  2. vm.memfd_noexec>=1 (kernel>=6.3): MFD_EXEC now passed with fallback.");
                    }
                }
                else
                {
...
#endif
        int rmemfd = _rsyscall.rmemfd_create(_rbuffer, MFD_CLOEXEC | MFD_ALLOW_SEALING | MFD_EXEC);
        if (rmemfd <= 0)
        {

            KITTY_LOGW("nativeInject: MFD_EXEC rejected (%s) — retrying without it (kernel < 6.3)",
                       _rsyscall.lastError().c_str());
            _rsyscall.clearLastError();
            rmemfd = _rsyscall.rmemfd_create(_rbuffer, MFD_CLOEXEC | MFD_ALLOW_SEALING);
        }
        if (rmemfd <= 0)
        {
...
inject_elf_info_t KittyInjector::emuInject(KittyIOFile &elfFile, bool *bCalldlerror)
{
    ...
    auto do_memfd_dlopen = [&]() -> void {
        ...
        if (rmemfd <= 0)
        {
            KITTY_LOGW("emuInject: MFD_EXEC rejected (%s) — retrying without it (kernel < 6.3)",
                       _rsyscall.lastError().c_str());
            _rsyscall.clearLastError();
            rmemfd = _rsyscall.rmemfd_create(_rbuffer, MFD_CLOEXEC | MFD_ALLOW_SEALING);
        }
