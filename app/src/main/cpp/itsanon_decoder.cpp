...
static void fireError(DecoderCtx* ctx, int code, const char* message) {
    ...
    jstring msg = env->NewStringUTF(message ? message : "ItsAnon pipeline error");
...
static bool buildPipeline(DecoderCtx* ctx) {
    destroyPipeline(ctx);
    ctx->pipeline = gst_pipeline_new("ecomcam-itsanon-pipeline");
...
static void runDecoder(DecoderCtx* ctx) {
    /* [V91] every ItsAnonDecoder.open() starts a fresh runDecoder thread, so
...
extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void*) {
    ...
    jclass local = env->FindClass("com/itsme/itsanon/media/ItsAnonDecoder$FrameCallback");
    ...
    LDECI("JNI_OnLoad: ItsAnon decoder initialized and callback methods cached");
    return JNI_VERSION_1_6;
}

extern "C" JNIEXPORT jlong JNICALL
Java_com_itsme_itsanon_media_ItsAnonDecoder_open(JNIEnv* env, jclass,
...
Java_com_itsme_itsanon_media_ItsAnonDecoder_close(JNIEnv* env, jclass, jlong handle) {
...
Java_com_itsme_itsanon_media_ItsAnonDecoder_hotSwap(JNIEnv* env, jclass,
...
Java_com_itsme_itsanon_media_ItsAnonDecoder_getWidth(JNIEnv*, jclass, jlong handle) {
    auto* ctx = reinterpret_cast<DecoderCtx*>(handle);
    return ctx ? ctx->width.load() : 0;
}

extern "C" JNIEXPORT jint JNICALL
Java_com_itsme_itsanon_media_ItsAnonDecoder_getHeight(JNIEnv*, jclass, jlong handle) {
    auto* ctx = reinterpret_cast<DecoderCtx*>(handle);
    return ctx ? ctx->height.load() : 0;
}

extern "C" JNIEXPORT jboolean JNICALL
Java_com_itsme_itsanon_media_ItsAnonDecoder_isUsingHardwareDecoder(JNIEnv*, jclass, jlong handle) {
    auto* ctx = reinterpret_cast<DecoderCtx*>(handle);
