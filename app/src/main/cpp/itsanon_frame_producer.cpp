...
#define TAG "itsanon/frame_producer"
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO,  TAG, __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, TAG, __VA_ARGS__)
#define LOGW(...) __android_log_print(ANDROID_LOG_WARN,  TAG, __VA_ARGS__)

#define FRAME_PRODUCER_VERSION "ITSANON.70-GZDROPBOX-20260914"
...
static int create_ashmem_ring(int w, int h) {
    ...
    int fd = ASharedMemory_create("itsanon_ring", total);
    if (fd < 0) {
...
static int send_fd_to_hook(int fd) {
    ...
    const socklen_t addr_len = static_cast<socklen_t>(
        offsetof(struct sockaddr_un, sun_path) + sizeof(ITSANON_SOCKET_NAME) - 1);
...
static bool buildProducerPipeline() {
    ...
    if (!g_pipeline || !g_source_element || !g_video_queue || !g_video_convert ||
        ...
        LOGE("ItsAnon producer element creation failed");
        return false;
    }
    ...
    if (!gst_element_link_many(g_video_queue, g_video_convert, g_video_scale,
        ...
        LOGE("ItsAnon producer branch linking failed");
        return false;
    }
...
JNI_OnLoad(JavaVM *vm, void *) {
    ...
    gst_init(nullptr, nullptr);
    if (gst_android_load_gio_modules) gst_android_load_gio_modules();
    LOGI("[%s] JNI_OnLoad: ItsAnon producer initialized", FRAME_PRODUCER_VERSION);
    return JNI_VERSION_1_6;
}

JNIEXPORT jint JNICALL
Java_com_itsme_itsanon_hooks_NativeFrameProducer_nativeStart(JNIEnv *env,
                                                              jobject ,
                                                              jstring jSourcePath) {
...
Java_com_itsme_itsanon_hooks_NativeFrameProducer_nativeStop(JNIEnv * ,
                                                             jobject ) {
    pthread_mutex_lock(&g_init_lock);
    g_running.store(false);
    if (g_thread_valid) {
...
JNIEXPORT void JNICALL
Java_com_itsme_itsanon_hooks_NativeFrameProducer_nativeSetOverlayParams(
        JNIEnv * , jobject ,
...
JNIEXPORT void JNICALL
Java_com_itsme_itsanon_hooks_NativeFrameProducer_nativeSetRotation(
        JNIEnv * , jobject ,
        jint degrees) {
...
JNIEXPORT void JNICALL
Java_com_itsme_itsanon_hooks_NativeFrameProducer_nativeSetChromaOverride(
        JNIEnv * , jobject ,
        jint overrideIsNv21) {
...
JNIEXPORT void JNICALL
Java_com_itsme_itsanon_hooks_NativeFrameProducer_nativeSetPaused(
        JNIEnv * , jobject ,
        jboolean paused) {
