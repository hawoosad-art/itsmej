package com.itsme.amkush.console
...
object CloakManager {
    ...
    private const val TAG = "amkush/cloak"
    private const val SCRIPT = "/data/local/tmp/amkush_cloak.sh"
    private const val DUMMY = "/data/local/tmp/.amkush_dummy"
    ...
    private fun ensureCloakBin(ctx: Context): String? {
        ...
        val asset = if (is64) "system/bin/amkush_cloak64" else "system/bin/amkush_cloak32"
        val dest = File(ctx.filesDir, "amkush_cloak")
        try {
    ...
    private fun stageScript(): String? {
         ...
         * compiled amkush_cloak binary — but the plaintext still leaked our
