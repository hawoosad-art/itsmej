package com.itsme.itsanon.manager
...
class ModuleManager(private val context: Context) {
    ...
    companion object {
        ...
        const val HOOK_BUILD_FILE = "/data/local/tmp/itsanon_hook_build"
     ...
     * abstract AF_UNIX socket "\0itsanon_frame_fd". ipc_socket_start() binds it as
     ...
     * "@itsanon_frame_fd" there means the hook is listening. This is the ONLY signal
    ...
    private fun hookSocketLiveness(): Boolean? {
        ...
        return unix.contains("@itsanon_frame_fd")
    }
    ...
    private fun detectHookInCameraserver(): Triple<Boolean, String, String> {
        ...
        // "\0itsanon_frame_fd", so frame_producer gets "Connection refused" on every
        ...
        // "\0itsanon_frame_fd" (shown as "@itsanon_frame_fd" in /proc/net/unix),
        ...
        val hookLine = if (isHooked) {
            when {
                sockLive == true ->
                    "injected (pid=$csPid, IPC live: itsanon_frame_fd)"
                mapsOutput.contains("libhookProxy") ->
    ...
    private fun getLogTail(): String {
        val tags = listOf(
            "itsanon/main", "itsanon/hook_proxy", "itsanon/watchdog", "itsanon/service",
            "itsanon/frame_inject", "itsanon/frame_source", "itsanon/frame_producer",
            "itsanon/ipc_socket", "itsanon/sym_resolver", "itsanon/stream_map",
            "ModuleManager"
        ).joinToString(" ") { "-s $it:V" }
     ...
     * The producer connects to the abstract socket "\0itsanon_frame_fd" bound by
    ...
    private fun ipcSelinuxRules(): List<String> =
        com.itsme.itsanon.security.SelinuxIpc.ipcRules()
    ...
    private fun applyLivePolicyRules(rules: List<String>, what: String) {
        if (com.itsme.itsanon.security.SelinuxIpc.applyIpcRules("$TAG/$what")) {
    ...
    fun ensureFreshAssets() {
        ...
        extractAssetToDest("system/bin/itsanon_injector64", java.io.File(context.filesDir, "itsanon_injector64").absolutePath)
        extractAssetToDest("system/bin/itsanon_injector32", java.io.File(context.filesDir, "itsanon_injector32").absolutePath)
        extractAssetToDest("system/lib64/libhookProxy.so", java.io.File(context.filesDir, "libhookProxy.so").absolutePath)
        shellSilent("chmod 0755 ${java.io.File(context.filesDir, "itsanon_injector64").absolutePath} ${java.io.File(context.filesDir, "itsanon_injector32").absolutePath}")
        writeAssetStamp()
    }
    ...
    private suspend fun injectNowLocked(): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            val (injectorAsset, hookAsset) = if (csIs64) {
                "system/bin/itsanon_injector64" to "system/lib64/libhookProxy.so"
            } else {
                "system/bin/itsanon_injector32" to "system/lib/libhookProxy.so"
            }
            ...
            shellSilent("pkill -f itsanon_injector 2>/dev/null || true")
            Thread.sleep(400L)
            ...
            if (stamp != null) {
                 ...
                 * extraction can leave itsanon_injector64 without +x, and then
            ...
            if (hooked) {
                ...
                try {
                    ...
                    val alreadyApplied = bootId.isNotEmpty() &&
                        shellSilent("cat /data/local/tmp/.itsanon_selrule_bootid 2>/dev/null").trim() == bootId
                        ...
                        if (anyOk && bootId.isNotEmpty()) {
                            try {
                                Runtime.getRuntime().exec(arrayOf(
                                    "su", "-c", "echo '$bootId' > /data/local/tmp/.itsanon_selrule_bootid"
                ...
                try {
                    com.itsme.itsanon.logging.CameraserverDumpUploader.maybeUpload(context)
                } catch (_: Exception) {}
    ...
    suspend fun startWatchdogDaemon(): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            val (wdAsset, injAsset, hookAsset) = if (wdIs64) {
                Triple("system/bin/watchdog_daemon64", "system/bin/itsanon_injector64", "system/lib64/libhookProxy.so")
            } else {
                Triple("system/bin/watchdog_daemon32", "system/bin/itsanon_injector32", "system/lib/libhookProxy.so")
            }
