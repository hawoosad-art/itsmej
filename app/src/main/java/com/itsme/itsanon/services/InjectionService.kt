package com.itsme.itsanon.services
...
import com.itsme.itsanon.ipc.ISurfaceInjector
import com.itsme.itsanon.ipc.UnixSocketServer
import com.itsme.itsanon.ipc.RemoteConfig
import com.itsme.itsanon.router.SurfaceRouter
...
class InjectionService : Service() {
            ...
            decoderExecutor.execute {
                ...
                synchronized(this@InjectionService) {
                    ...
                    if (h != 0L) {
                        ...
                        ItsAnonDecoder.hotSwap(h, resolved)
                    } else {
    ...
    override fun onCreate() {
        ...
        try { File("/sdcard/Android/media/com.itsme.itsanon").mkdirs() } catch (_: Throwable) {}


        UnixSocketServer.start()
        Logger.i(Logger.INJECTION, "$TAG created — ItsAnon decoder + Unix socket server ready")
    ...
    private val frameCallback = object : ItsAnonDecoder.FrameCallback {
    ...
    private fun startOrRestartDecoder(rawUrl: String) {
        ...
        Logger.d(Logger.INJECTION, "$TAG ItsAnonDecoder.open url=$url")
        val handle = ItsAnonDecoder.open(url, frameCallback)
        if (handle == 0L) {
            Logger.e(Logger.INJECTION, "$TAG ItsAnonDecoder.open FAILED for: $url")
            Log.e("ECOMCAM", "InjectionService: ItsAnonDecoder.open FAILED url=$url")
        } else {
            decoderHandle = handle
            Logger.d(Logger.INJECTION, "$TAG decoder running handle=$handle  hw=${ItsAnonDecoder.isUsingHardwareDecoder(handle)}  url=$url")
